            ...
            partial class C
            {
                public static partial global::ArgumentParsing.Results.ParseResult<global::EmptyOptions> ParseArguments({{parameterType}} s)
                {
                    ...
                    foreach (string arg in s)
                    {
                        ...
                        bool startsOption = arg.StartsWith('-') && arg.Length > 1;
                        ...
                        if (arg.StartsWith("--"))
                        {
                            ...
                            global::System.ReadOnlySpan<char> longOptionName = slice[longArgSplit[0]];
                            switch (longOptionName)
                            {
                                default:
                                    errors ??= new();
                                    errors.Add(new global::ArgumentParsing.Results.Errors.UnknownOptionError(longOptionName.ToString(), arg));
                                    if (written == 1)
                                    {
                                        state = -1;
                                    }
